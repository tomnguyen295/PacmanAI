    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, len(thresholds)*2))
        ...
        for i in range(self.numGames):
            if games[i].state.isWin() and not games[i].agentTimeout:
                totalPoints += 2
                self.addMessage("Win at least once: 2 points")
                break
