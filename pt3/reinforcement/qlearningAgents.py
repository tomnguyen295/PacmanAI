...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if not legalActions:
            return None
        actionTarget = [None, -9999999999999999]
        for action in legalActions:
            qvalue = self.getQValue(state, action)
            if qvalue > actionTarget[1]:
                actionTarget = action, qvalue
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.QValues[(state, action)] = (1-self.alpha) * qVal + self.alpha * (reward + self.discount * self.getValue(nextState) )
        # util.raiseNotDefined()
