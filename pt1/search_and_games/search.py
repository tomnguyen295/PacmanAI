...
def breadthFirstSearch(problem):
    ...
    while (not frontier.isEmpty()):
        ...
        for action in problem.getActions(node.state):
            child = Node(problem.getResult(node.state,action), node, action, problem.getCost(node.state,action))
    ...
    while (not frontier.isEmpty()):
        ...
        for action in problem.getActions(node.state):
            ...
            if problem.goalTest(child.state):
                ...
                actions.reverse()
                return actions
            if child.state not in explored and child.getDepth() <= limit:
                frontier.push(child)
                explored.add(child.state)
...
def iterativeDeepeningSearch(problem: SearchProblem):
    ...
    limit = 0
    while True:
        actions = depthLimitedSearch(problem, limit)
        if (actions):
            return actions
        limit += 1
